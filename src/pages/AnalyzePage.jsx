...
export default function AnalyzePage() {
  ...
  const analyze = async (kw) => {
    ...
    try {
      const res = await axios.post("https://predictx-7ylf.onrender.com/api/analyze-trend", { keyword: target });
      setResult(res.data.data);
      setKeyword(target);
      toast.success("Analysis complete", `Trend score: ${res.data.data.trendScore}/100 · ${res.data.data.recommendation}`);
    } catch {
      setError("Could not reach the API server at https://predictx-7ylf.onrender.com.");
      toast.error("Analysis failed", "Could not connect to the API server.");
    } finally {
      setLoading(false);
    }
  };
